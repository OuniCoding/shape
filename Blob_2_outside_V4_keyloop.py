...
param = set_blob_param(color_t, 'Settings/oblob-param.xml') # Settings/oblob-param-newcam.xml# add D:/project/bottlecap/code/
...
#-----------------------------------------------------------------------
    ...
    if white_pixels < min_pixels:#600000: #800000:
        cv.putText(frame_res, str(len(corner_points)) + ' ' + str(len(slopes_pt)) + ' Deformation', (5, 40), cv.FONT_HERSHEY_PLAIN,
                   1.2,(0, 0, 255), 1)
        for i in range(len(slopes_pt)):
